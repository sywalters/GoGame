...
class GoAI:
        ...
        for row in range(game.board_size):
            for col in range(game.board_size):
                if game.board[row][col] == Player.EMPTY:
                    ...
                    if not game.is_suicide_move(row, col, self.player) and not game.violates_ko(row, col):
                        valid_moves.append((row, col))
        return valid_moves
    
    def _get_corner_moves(self, game: GoGame, valid_moves: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
        ...
        corner_positions = [
            ...
            (2, 3), (3, 2), (2, board_size-4), (3, board_size-3),
            (board_size-3, 3), (board_size-4, 2), (board_size-3, board_size-4), (board_size-4, board_size-3)
        ]
    ...
    def _evaluate_move(self, game: GoGame, move: Tuple[int, int]) -> float:
        ...
        score += self._evaluate_captures(temp_game, move) * self.config["capture_weight"]
        
        # 2. Liberty evaluation
        score += self._evaluate_liberties(temp_game, move) * self.config["liberty_weight"]
        
        # 3. Territory evaluation
        score += self._evaluate_territory(temp_game, move) * self.config["territory_weight"]
    ...
    def _evaluate_territory(self, temp_game: GoGame, move: Tuple[int, int]) -> float:
        ...
        for dr in range(-influence_radius, influence_radius + 1):
            for dc in range(-influence_radius, influence_radius + 1):
                ...
                if (0 <= new_row < temp_game.board_size and 
                    0 <= new_col < temp_game.board_size and
                    temp_game.board[new_row][new_col] == Player.EMPTY):
    ...
    def _evaluate_patterns(self, game: GoGame, move: Tuple[int, int]) -> float:
        ...
        if row == 0 or row == game.board_size - 1 or col == 0 or col == game.board_size - 1:
            score -= 2
        if row == 1 or row == game.board_size - 2 or col == 1 or col == game.board_size - 2:
            score -= 1
    ...
    def _select_top_moves(self, move_scores: List[Tuple[Tuple[int, int], float]]) -> List[Tuple[int, int]]:
    ...
    def _add_randomness(self, moves: List[Tuple[int, int]]) -> Tuple[int, int]:
